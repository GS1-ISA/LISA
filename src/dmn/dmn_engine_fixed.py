"""
DMN Execution Engine (Fixed Version)
...
class DMNEngine:
    ...
    def _evaluate_rule(self, rule: Rule, decision_table: DecisionTable,
        ...
        for input_clause in decision_table.input_clauses:
            ...
            if input_expression is None or input_expression.strip() == "":
                continue  # Empty input entry means "don't care"

            # Get the actual input value from the input data using the input clause expression
            ...
            # Check if the actual value matches the rule's input expression
            if not self._matches_rule_expression(input_expression, actual_value):
    ...
    def _matches_rule_expression(self, rule_expression: str, actual_value: Any) -> bool:
        """Check if actual input value matches the rule expression."""
        rule_expression = rule_expression.strip()

        # Handle "-" (don't care) expressions
        if rule_expression == "-":
            return True

        # Handle direct equality
        if rule_expression == str(actual_value):
            return True

        # Handle boolean values
        if isinstance(actual_value, bool):
            if rule_expression.lower() in ["true", "false"]:
                return str(actual_value).lower() == rule_expression.lower()
            return False

        # Handle numeric comparisons
        if isinstance(actual_value, (int, float)):
            return self._evaluate_numeric_comparison(rule_expression, actual_value)

        # Handle string matching
        if isinstance(actual_value, str):
            return self._evaluate_string_matching(rule_expression, actual_value)

        return False

    def _evaluate_numeric_comparison(self, expression: str, value: Union[int, float]) -> bool:
        """Evaluate numeric comparison expressions in rules."""
        try:
            # Handle >=
    ...
    def _evaluate_string_matching(self, expression: str, value: str) -> bool:
        """Evaluate string matching expressions in rules."""
        # Handle comma-separated lists (OR condition)
        if "," in expression:
            options = [opt.strip() for opt in expression.split(",")]
