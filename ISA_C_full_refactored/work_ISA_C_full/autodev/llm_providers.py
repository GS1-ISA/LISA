...
def _coerce_json(text: str) -> Dict[str, Any]:
    try:
        ...
        return {"decision":"reflect","files":[],"patches":[],"notes":"non-json response","next_action":"fix-output"}
class LLMClient:
    def __init__(self):
    ...
    def complete_json(self, system: str, user: str) -> LLMResult:
        ...
        if kind == "gemini":
            ...
            resp = client.generate_content(prompt, generation_config={"temperature":0,"max_output_tokens":self.max_tokens}, safety_settings=[])
            text = resp.text or "{}"
            return LLMResult(_coerce_json(text))
        return LLMResult({"decision":"plan","files":[{"path":"out/dry_run.txt","content":"dry run"}],"patches":[],"notes":"dry","next_action":"implement"})
