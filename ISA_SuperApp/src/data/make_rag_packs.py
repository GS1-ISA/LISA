...
def pack_for_langchain(dirpath):
    dest = os.path.join(OUT, "langchain")
    idx = []
    for root, _, files in os.walk(dirpath):
        for f in files:
            if f.endswith(".md"):
                p = os.path.join(
                    dest,
                    os.path.relpath(os.path.join(root, f), start=dirpath).replace(os.sep, "__"),
                )
                os.makedirs(os.path.dirname(p), exist_ok=True)
                shutil.copy2(os.path.join(root, f), p)
                idx.append({"path": p, "metadata": {}})
    with open(os.path.join(dest, "index.json"), "w") as jf:
        json.dump(idx, jf, indent=2)


def pack_for_obsidian(dirpath):
    dest = os.path.join(OUT, "obsidian", ".obsidian")
