...
class VectorIndex:
    def __init__(
        self,
        storage_path: str = "storage/index.json",
    ...
    def _save(self):
        ...
        data = {
            "docs": self.docs,
            "vecs": self.vecs,
            "lex_df": dict(self.lex_df),
            "lex_vocab_docs": self.lex_vocab_docs,
        }
    ...
    def _lex_score(self, query: str, text: str) -> float:
        ...
        def tfidf(tf, tok):
            tfv = tf.get(tok, 0)
            df = self.lex_df.get(tok, 1)
            import math

            idf = math.log((self.lex_vocab_docs + 1) / (df + 0.5))
            return tfv * idf

        vq = [tfidf(tf_q, t) for t in vocab]
        vd = [tfidf(tf_d, t) for t in vocab]
        num = sum(a * b for a, b in zip(vq, vd))
        import math

        dq = math.sqrt(sum(a * a for a in vq)) or 1e-9
        ...
        if self.vecs:
            ...
            if qv and qv[0]:
                ...
                for i, doc in enumerate(self.docs):
                    s = _cosine(qvec, self.vecs[i])
                    scores.append((s, doc))
        if not scores:
            for doc in self.docs:
                s = self._lex_score(query, doc["text"])
                scores.append((s, doc))
