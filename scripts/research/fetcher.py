...
def fetch(
    ...
    *,
    ...
    if parsed.scheme in ("", "file"):
        ...
        rec = FetchRecord(url=str(path), ok=True, status=200, bytes=len(data), sha256=sha, fetched_at=time.time(), from_cache=True, note="local-file")
    ...
    if cache_file.exists():
        ...
        sha = _sha256_bytes(data)
        rec = FetchRecord(url=url, ok=True, status=200, bytes=len(data), sha256=sha, fetched_at=time.time(), from_cache=True, note="cache-hit")
        with audit_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(asdict(rec)) + "\n")
        return data, rec
    ...
    if not (allow_network or env_allow):
        rec = FetchRecord(url=url, ok=False, status=0, bytes=0, sha256="", fetched_at=time.time(), from_cache=False, note="network-disabled")
        with audit_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(asdict(rec)) + "\n")
        return None, rec
    ...
    if not _robots_allowed(url, user_agent=user_agent):
        rec = FetchRecord(url=url, ok=False, status=0, bytes=0, sha256="", fetched_at=time.time(), from_cache=False, note="robots-disallow")
        with audit_path.open("a", encoding="utf-8") as f:
            f.write(json.dumps(asdict(rec)) + "\n")
        return None, rec
    ...
    try:
        ...
        if ok:
            ...
            meta_file.write_text(json.dumps({"url": url, "status": r.status_code, "headers": dict(r.headers), "sha256": sha}, indent=2), encoding="utf-8")
        rec = FetchRecord(url=url, ok=ok, status=r.status_code, bytes=len(data), sha256=sha, fetched_at=time.time(), from_cache=False)
    except Exception as e:
        rec = FetchRecord(url=url, ok=False, status=0, bytes=0, sha256="", fetched_at=time.time(), from_cache=False, note=f"error:{e}")
        data = None
