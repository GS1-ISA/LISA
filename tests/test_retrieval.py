...
class TestQueryExpander:
    ...
    def query_expander(self):
        ...
        with patch('isa_superapp.retrieval.query_expansion.AutoModelForSeq2SeqLM') as mock_model, \
             patch('isa_superapp.retrieval.query_expansion.AutoTokenizer') as mock_tokenizer:
    ...
    def test_expand_query_basic(self, query_expander):
        ...
        query_expander.model.generate = Mock(return_value=[[101, 2023, 2003, 1037, 3231]])
        query_expander.tokenizer.decode = Mock(return_value="what is machine learning")
        query_expander.tokenizer.batch_decode = Mock(
            return_value=["what is machine learning", "machine learning algorithms", "machine learning applications"]
        )
    ...
    def test_expand_query_with_context(self, query_expander):
        ...
        query_expander.tokenizer.batch_decode = Mock(
            return_value=["Python function syntax", "how to define functions in Python", "Python function examples"]
        )
...
class TestReRanker:
    ...
    def reranker(self):
        ...
        with patch('isa_superapp.retrieval.re_ranking.CrossEncoder') as mock_cross_encoder:
            mock_model = Mock()
...
class TestHybridRetriever:
    ...
    def hybrid_retriever(self):
        ...
        with patch('isa_superapp.retrieval.hybrid_retriever.HybridVectorStore') as mock_store:
...
class TestMultiQueryRetriever:
    ...
    def multi_query_retriever(self):
        ...
        with patch('isa_superapp.retrieval.multi_query_retriever.QueryExpander') as mock_expander, \
             patch('isa_superapp.retrieval.multi_query_retriever.HybridVectorStore') as mock_store:
    ...
    async def test_retrieve_with_variants(self, multi_query_retriever):
        ...
        def mock_search(embedding, **kwargs):
            if "what are" in str(embedding):
                return [
                    SearchResult(
                        document=VectorDocument(id="doc_1", content="What are ML algorithms"),
                        score=0.95
                    )
                ]
                ...
                return [
                    SearchResult(
                        document=VectorDocument(id="doc_2", content="Types of ML algorithms"),
                        score=0.9
                    )
                ]
            else:
                return [
                    SearchResult(
                        document=VectorDocument(id="doc_3", content="ML algorithms guide"),
                        score=0.85
                    )
                ]
    ...
    async def test_retrieve_with_deduplication(self, multi_query_retriever):
        ...
        def mock_search(embedding, **kwargs):
            return [
                SearchResult(
                    document=VectorDocument(id="doc_1", content="Python programming guide"),
                    score=0.95
                ),
                SearchResult(
...
class TestContextualRetriever:
    ...
    def contextual_retriever(self):
        ...
        with patch('isa_superapp.retrieval.contextual_retriever.HybridVectorStore') as mock_store:
            mock_store_instance = Mock()
            mock_store.return_value = mock_store_instance
    ...
    async def test_retrieve_with_conversation_history(self, contextual_retriever):
        ...
        mock_results = [
            SearchResult(
                document=VectorDocument(
                    id="doc_1",
                    content="Recursion implementation examples in Python"
                ),
                score=0.95
            )
        ]
...
class TestRAGRetriever:
    ...
    def rag_retriever(self):
        ...
        with patch('isa_superapp.retrieval.rag_retriever.HybridVectorStore') as mock_store, \
             patch('isa_superapp.retrieval.rag_retriever.ReRanker') as mock_reranker, \
             patch('isa_superapp.retrieval.rag_retriever.ContextCompressor') as mock_compressor:
