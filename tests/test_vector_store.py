...
class TestVectorDocument:
    ...
    def test_vector_document_with_metadata_object(self):
        ...
        doc = VectorDocument(
            id="doc_789",
            content="Document with structured metadata",
            metadata=metadata
        )
...
class TestSearchResult:
    ...
    def test_search_result_creation(self):
        ...
        result = SearchResult(
            document=doc,
            score=0.95,
            metadata={"search_method": "cosine_similarity"}
        )
...
class TestEmbeddingProvider:
    ...
    def embedding_provider(self):
        ...
        with patch('isa_superapp.vector_store.embeddings.AutoModel') as mock_model, \
             patch('isa_superapp.vector_store.embeddings.AutoTokenizer') as mock_tokenizer:
            ...
            return EmbeddingProvider(model_name="sentence-transformers/all-MiniLM-L6-v2")
...
class TestChromaVectorStore:
    ...
    def chroma_store(self):
        ...
        with patch('isa_superapp.vector_store.chroma_store.chromadb') as mock_chroma, \
             patch('isa_superapp.vector_store.chroma_store.EmbeddingProvider') as mock_embedding:
    ...
    async def test_search(self, chroma_store):
        ...
        chroma_store.collection.query = Mock(return_value={
            "ids": [["doc_1", "doc_2"]],
    ...
    async def test_get_document(self, chroma_store):
        ...
        chroma_store.collection.get = Mock(return_value={
            "ids": ["doc_1"],
...
class TestPineconeVectorStore:
    ...
    def pinecone_store(self):
        ...
        with patch('isa_superapp.vector_store.pinecone_store.Pinecone') as mock_pinecone, \
             patch('isa_superapp.vector_store.pinecone_store.EmbeddingProvider') as mock_embedding:
    ...
    async def test_search(self, pinecone_store):
        ...
        # Mock query
        pinecone_store.index.query = Mock(return_value={
            "matches": [
                {
                    "id": "doc_1",
                    "score": 0.9,
...
class TestHybridVectorStore:
    ...
    def hybrid_store(self):
        ...
        with patch('isa_superapp.vector_store.hybrid_store.ChromaVectorStore') as mock_chroma, \
             patch('isa_superapp.vector_store.hybrid_store.PineconeVectorStore') as mock_pinecone, \
             patch('isa_superapp.vector_store.hybrid_store.SparseEmbeddingProvider') as mock_sparse:
    ...
    async def test_add_documents_hybrid(self, hybrid_store):
        ...
        documents = [
            VectorDocument(
                id="doc_1",
                content="First document for hybrid indexing"
            ),
            VectorDocument(
                id="doc_2",
                content="Second document for hybrid indexing"
            )
        ]
    ...
    async def test_search_hybrid(self, hybrid_store):
        ...
        dense_results = [
            SearchResult(
                document=VectorDocument(id="doc_1", content="Dense result 1"),
                score=0.9
            ),
            SearchResult(
                document=VectorDocument(id="doc_2", content="Dense result 2"),
                score=0.8
            )
        ]
