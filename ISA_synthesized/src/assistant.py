...
setup_logging(); log=logging.getLogger("assistant")
class AssistantOrchestrator:
    def __init__(self, memory: KnowledgeGraphMemory|None=None, reasoner: SequentialReasoner|None=None, retriever: VectorIndex|None=None):
    ...
    def ask(self, question: str, use_memory: bool = True, max_steps: int = 5, explain: bool = False):
        context = ""
        if use_memory:
            ...
            try:
                ...
                for h in vec_hits:
                    snippets.append(f"[SIM:{h.get('id')}] score={h.get('score'):.3f} :: " + h.get('text','')[:200])
