...
def job_refresh(orch: AssistantOrchestrator):
    ...
    try: ingest_eurlex_esg(orch.memory, "ESG", limit=5)
    except Exception as e: log.warning("EUR-Lex ingest failed: %s", e)
    try: ingest_efrag_news(orch.memory, limit=5)
    except Exception as e: log.warning("EFRAG ingest failed: %s", e)
    try: ingest_gs1_standards_log(orch.memory)
    except Exception as e: log.warning("GS1 log ingest failed: %s", e)
    try: orch.rebuild_index_from_memory(); log.info("Vector index rebuilt.")
    except Exception as e: log.warning("Index rebuild failed: %s", e)
def main():
    parser = argparse.ArgumentParser(description="ISA Scheduler")
    parser.add_argument("--interval-min", type=int, default=360, help="Run refresh job every N minutes (default 360)")
